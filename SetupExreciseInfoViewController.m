...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.editMode == YES) {
        ...
        switch (self.selectedEvent.eventCategory) {
            case kWeights:
                event = [self.coreDataHelper fetchDataFor:@"DefaultWeightLifting"  withPredicate:@{@"propertyName" : @"eventName", @"value" : self.selectedEvent.eventName} sortKey:nil scheduleInfo:nil];
            ...
            default:
                event = [self.coreDataHelper fetchDataFor:@"DefaultAerobic" withPredicate:@{@"propertyName" : @"eventName", @"value" : self.selectedEvent.eventName} sortKey:nil scheduleInfo:nil];
    ...
    if (([self.exerciseName.text length] > 0) && (self.categoryCode < self.exerciseCategoryCopy.count))
    {
        ...
        if (eventSelectedFromLAvailableList == NO  && self.editMode == NO)
        {
            ...
            switch (self.categoryCode) {
                case kWeights:
                    event = [self.coreDataHelper fetchDataFor:@"DefaultWeightLifting" withPredicate:@{@"propertyName" : @"eventName", @"value" : self.exerciseName.text} sortKey:nil scheduleInfo:nil];
                ...
                default:
                    event = [self.coreDataHelper fetchDataFor:@"DefaultAerobic" withPredicate:@{@"propertyName" : @"eventName", @"value" : self.exerciseName.text} sortKey:nil scheduleInfo:nil];
